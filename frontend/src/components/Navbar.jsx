    ...
    <header className="w-full bg-paper/95 dark:bg-ink-dark/95 backdrop-blur-md border-b border-line dark:border-line-dark sticky top-0 z-50 transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* User Minimal Pill */}
          {user && (
            <div className="hidden sm:flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-paper-raised dark:bg-surface-dark border border-line dark:border-line-dark">
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl bg-paper-raised dark:bg-surface-dark border border-line dark:border-line-dark text-ink-soft dark:text-ink-soft-dark hover:text-ink dark:hover:text-paper-dark hover:border-ink/30 transition cursor-pointer shadow-xs"
            title="Toggle theme"
          >
          ...
          {user && (
            <button
              ...
              className="flex items-center gap-2 bg-paper-raised dark:bg-surface-dark border border-line dark:border-line-dark hover:border-flame/40 text-ink-soft dark:text-ink-soft-dark hover:text-flame px-3.5 py-2.5 rounded-xl text-xs font-mono font-semibold transition cursor-pointer shadow-xs"
      ...
      {mobileMenuOpen && (
        <nav className="md:hidden flex flex-col gap-1.5 px-6 pb-4 bg-paper-raised dark:bg-surface-dark border-t border-line dark:border-line-dark">
          {navLinks.map((link) => {
            const Icon = link.icon;
            const isActive = location.pathname === link.path;
            return (
              <Link
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-lg font-mono text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-flame text-white'
                    : 'text-ink-soft dark:text-ink-soft-dark hover:bg-paper dark:hover:bg-ink-dark'
                }`}
              >
                <Icon size={15} />
                <span>{link.name}</span>
